...
let filter-pattern =  "^(gl[A-Z]|GL_|glad[A-Z])"
vvv bind glad-header
do
    let header =
    ...
    vvv bind define
    do
        using header.define filter filter-pattern
        locals;
    unlet header
    locals;
...
bindgen.from-include-scope glad-header "" transformers
